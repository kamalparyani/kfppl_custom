...
function fetchOverdue(frm, source) {
    ...
    frappe.call({
        ...
        callback: function(r) {
            ...
            // --- Msgprint behavior (only when customer changes), now showing *per-invoice* grace info ---
            if (source === "customer" && rows.length) {
                const fmt = inv => {
                    const dueStr    = frappe.datetime.str_to_user(inv.due_date);
                    const graceStr  = inv.invGrace ? `; Grace ${inv.invGrace}d until ${frappe.datetime.str_to_user(inv.deadline)}` : "";
                    return `<b>${inv.name}</b>: ₹${inv.outstanding_amount} (Due ${dueStr}${graceStr})`;
                };

                // Keep original headline, but content is accurate per invoice
                let msg = __("Customer has overdue invoices:") + "<br>";
                msg += rows.map(row => {
                    // rows doesn't have invGrace/deadline; recreate for display using same logic
