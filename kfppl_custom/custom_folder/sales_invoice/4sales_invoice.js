...
// Ensure dashboard is mounted before setting headline
function ensureDashboardReady(frm, fn) {
    // If dashboard object isn't ready yet, wait a tick
    if (!frm.dashboard || !frm.dashboard.wrapper) {
        setTimeout(() => ensureDashboardReady(frm, fn), 60);
        return;
    }
    // Some renders need a post-AJAX tick too
...
function fetchOverdue(frm, source) {
    ...
    frappe.call({
        ...
        callback: function(r) {
            ...
            // --- Msgprint behavior (only when customer changes), per-invoice grace info ---
            if (source === "customer" && rows.length) {
                let msg = __("Customer has overdue invoices:") + "<br>" + rows.map(row => {
...
frappe.ui.form.on('Sales Invoice', {
    // Fires when form UI is fully rendered; safe place to set dashboard headline
