...
def execute(filters=None):
    ...
    # ---- Base GL query (use SI for time ordering)
    # Note: tabGL Entry has no posting_time; use si.posting_time and fall back to TIME(si.creation).
    base_sql = f"""
        SELECT
    ...
    # ---- Helpers (stick to v15-available utils)
    fmt_html = frappe.utils.escape_html
    fmt_date = frappe.utils.formatdate
    fmt_curr = frappe.utils.fmt_money

    # float precision helper (use site default "float_precision" or fallback to 3)
